    ...
    Gauge.prototype.renderStaticLabels = function(staticLabels, w, h, radius) {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (value.label !== void 0) {
          ...
          if ((!this.options.limitMin || value >= this.minValue) && (!this.options.limitMax || value <= this.maxValue)) {
              if ( j == 4){
                 nextAngle = this.getAngle(2999)
               } else {
                 nextAngle = this.getAngle(ref[j+1])
               }

            calc_angle = (nextAngle - this.getAngle(value) ) / 2 + this.getAngle(value)
            rotationAngle = calc_angle - 3 * Math.PI / 2;
            this.ctx.rotate(rotationAngle);
            this.ctx.fillText(labels[j], 0, -radius - this.lineWidth / 2);
            //this.ctx.fillText("AOJEOFIJEOIFJEF", 0, -radius - this.lineWidth / 2);
